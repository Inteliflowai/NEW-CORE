...
- **P1 Pilot — full Pro baseline (adds onto M-Slice).** + **Risk Index** + **full Super TELI** + **Spark (Pro+)** + **full "did it work" Confirm** (reteach-effectiveness rollups) + **Parent** dashboard + **School Admin** (adoption + Risk Index) + **full Super Admin** (schools / licensing / maintenance / changelog) + **misconception taxonomy** + full media. This is everything SCOPE locks for the Pro pilot (all 5 roles full depth).
- **Security minimum (PILOT-BLOCKING — never a fast-follow):** object-level guards + the meta-test; licensing/anti-piracy core (HMAC keys, domain lock, **seat enforcement incl. `trialing`**, tenant isolation, **true read-only maintenance-mode enforcement** — not banner-only); Spark contract security (auth, **idempotency state machine**, allow-listed URLs, **FERPA delete/forget action**, **a conservative per-key/per-school rate limiter**). These gate any slice that exposes the relevant surface.
