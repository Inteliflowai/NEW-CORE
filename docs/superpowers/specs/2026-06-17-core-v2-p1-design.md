...
One job: "Are teachers and students actually being helped?" Adoption + risk, nothing operational. **`guardSchoolAdmin` (§1.4a).** Mine `admin/analytics/`, `admin/teachers/`, `admin/alerts/`; Risk Index math from §4 (Pro — gate with `checkFeature`). Calm, credible information design — a clownish admin dashboard kills the sale (§16). **Scope guardrail:** longitudinal/cohort-benchmarking/custom-report screens are Enterprise (P2) — out of pilot.

**AGGREGATE-BY-DEFAULT (FERPA/LGPD minimum-necessary — first-class, not a UI afterthought).** The admin landing state shows **aggregate counts and teacher-level adoption only — never a named-student roster on load**:
- **Adoption** (weekly logins, CORE-recommended actions taken, time-to-first-value — metrics #1/#2 at school scope) + a **count** of at-risk students this week (e.g. "7 students flagged · 2 teachers with no action") and a teacher-level "who hasn't acted" view (teacher names, not student names).
- **Named at-risk drill-down is a DELIBERATE, permissioned action** — the admin must explicitly select a risk bucket ("See flagged students") to reveal student names; named rows are **never the default rendering**. Each revealed row still resolves to "which teacher owns this / has anyone acted." **Primary action: nudge a teacher** (from the aggregate); opening the named at-risk roster is the secondary, deliberate step. The data layer already supports both (the roster API returns names; the *landing* must request only aggregates).
- **Equity / consistency across sections** (E3): a "where students need support" view — which skills have ≥N% of students below On-Track across ≥2 sections — framed as **support needs, not teacher report cards** ("which teachers are failing" framing is explicitly forbidden). Reads `skill_learning_state` aggregated by `school_id`+`skill_id` (RLS already permits school-admin); aggregate counts, drill-down to names only on deliberate action.
- **Board / parent-community summary** (E5): a read-only school-level roll-up (adoption rate, % on-track vs flagged, top support needs by skill — **all aggregate, zero named students**), exportable as a shareable link/PDF. Detailed layout deferred to Plan 4, but it is a **first-class admin sub-view**, not an afterthought.

These four (Adoption, aggregate Risk count + named drill-down, Equity-across-sections, Board summary) are the School Admin surface. Detailed screen design is Plan 4; the **aggregate-first / deliberate-drill-down discipline is mandated here** so Plan 4 cannot build a names-on-load dashboard.
