...
function admin({ quizzes, questions }: { quizzes: { id: string }[]; questions: { skill_id: string | null; skills: { id: string; name: string } | { id: string; name: string }[] | null }[] }) {
  return {
    from: (table: string) => {
      if (table === 'quizzes') {
...
describe('resolveLessonSkills', () => {
  ...
  // FIX 9: PostgREST may return the skills embed as an array — normalize correctly
  it('FIX 9: handles skills returned as an array (PostgREST embed variation)', async () => {
    const out = await resolveLessonSkills(
      admin({
        quizzes: [{ id: 'q1' }],
        questions: [
          // skills returned as an array of one (PostgREST embed variation)
          { skill_id: 'frac', skills: [{ id: 'frac', name: 'Fractions' }] },
          { skill_id: 'dec', skills: { id: 'dec', name: 'Decimals' } }, // object form still works
        ],
      }),
      'lesson1',
    );
