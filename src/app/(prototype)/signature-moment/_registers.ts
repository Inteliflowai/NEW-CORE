...
// moment. Same four-beat, three feelings. Motion is pulled from the shared
// coachMotion module (src/lib/design/coachMotion.ts → COACH_MOTION), which
// itself sources values from tokens.motion. Colours from the role/intensity
// theming (data-role / data-intensity). Copy lines are DRAFT → Barb
// (STRINGS-FOR-BARB.md §Signature-Moment).
import { COACH_MOTION, type CoachMotionConfig } from '@/lib/design/coachMotion';

export type RegisterKey = 'student' | 'teacher' | 'parent';

export interface Register extends CoachMotionConfig {
  key: RegisterKey;
  label: string;
