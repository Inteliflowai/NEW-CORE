...
**NOW — the "meat and potatoes" V1-parity feature build (see [[v2-parity-program-meat-and-potatoes]] memory).** Data-producers first, epics **1→4**. **Status as of 2026-06-21:**
...
3. **Teacher completion — ◀ IN PROGRESS (Epic 3, started 2026-06-21).** gradebook/alerts/insights/high-fives are 10-line stubs; the data now flows. **Decomposed + design LOCKED — see [[v2-epic3-teacher-screens]]:** depth = **HYBRID** (full gradebook + lean others). **3a — FULL gradebook FIRST** (Barb weights it heavily): assignment-column grid + clearly-separated diagnostic-quiz section + click-cell drill-in w/ grade override (`homework_attempts.teacher_score` EXISTS, migration 0004 — no new migration) + reteach toggle. **3b — LEAN Alerts + High-Fives + Insights** next. Grounding: `docs/superpowers/plans/grounding/2026-06-21-teacher-completion/`. **NEXT = write the 3a gradebook spec → writing-plans → SDD.**
