...
**NOW — the "meat and potatoes" V1-parity feature build (see [[v2-parity-program-meat-and-potatoes]] memory).** Marvin pivoted off cosmetics. A V1↔V2 parity audit found the binding constraint: **V2's engines + teacher screens are built but STARVE — no student work produces signals, so Today/roster/student-drill/growth/parent all render against empty data.** Build sequence (data-producers first), epics **1→4 in order**:
1. **Student Quiz Runner** ← STARTING. The grader (`api/attempts/[attemptId]/submit/route.ts`, 383 lines) + `adapt/route.ts` ALREADY EXIST — net-new is the start/heartbeat/forfeit-resume/study-guide/quiz-history routes + the timed runner UI + `useEventTracker` emitters; it lights up the whole signals pipeline. Grounding: `docs/superpowers/plans/grounding/2026-06-20-quiz-runner.md`. Open Q: does a `quiz_attempts`/`questions` schema exist for the grader, or must we create it?
2. **Non-SPARK Assignment Player** (the flagship + pedagogical leap: Teli tutor, hint ladder, canvas, TTS, autosave, graded submit; V1 `student/homework` ~1557 lines) — after the Quiz Runner, to reuse its plumbing.
3. **Teacher completion** (gradebook/alerts/insights/high-fives are 10-line stubs).
4. **Parent dashboard + AI narrative** (9-paragraph summary over already-ported leak-guard primitives).
